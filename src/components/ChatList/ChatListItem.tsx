...
export default function ChatListItem({ props: { data } }: PropsInterface) {
    ...
    const { currentChatUser, setCurrentChatUser, setContactPage, userInfo } = useAppContext();
            ...
            <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
                ...
                <div className="flex border-b border-conversation-border pb-2 pt-1 p3-2">
                    <div className="flex justify-between w-full">
                        <span className="text-secondary line-clamb-1 text-sm ">{userInfo?.id !== data?.id ? data?.about || "\u00A0" : "Message Yourself"}</span>
                    </div>
                </div>
            </div>
        </div>}
